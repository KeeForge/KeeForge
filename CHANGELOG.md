...
- iOS, Mac App Store (MAS), and direct-download Mac use one `release/{major}.{minor}` branch, marketing version, monotonically increasing repo build number, immutable `rc/{version}-b{repoBuild}` tag, commit SHA, and source tree. A platform-only code fix respins all three artifacts.
- All four product targets (`KeeForge`, `KeeForgeAutoFill`, `KeeForgeMac`, `KeeForgeMacAutoFill`) carry identical `MARKETING_VERSION` and `CURRENT_PROJECT_VERSION`. Xcode Cloud may assign separate iOS and Mac TestFlight build numbers; record them instead of forcing them to equal the repo build.
- iOS and Mac TestFlight preparation/soak run in parallel. Neither blocks the other's preparation, but final promotion requires both App Store builds and the direct artifact to come from the same accepted RC. Never re-archive after soaking.
- Xcode Cloud archives/uploads may run automatically, but external TestFlight distribution is manual only after all cloud and local gates and both exact Mac artifact checks are accepted; first Beta App Review and each platform's external distribution require separate action-time confirmation.
- App Store Connect is intended to hold one KeeForge app record with separate iOS and macOS version records, builds, screenshots, metadata, and review submissions. Adding macOS and enabling universal purchase remain pending one-time ASC setup; the recommended default is universal purchase. Hold both with manual release for the first coordinated launch; phased release is optional and not yet chosen.
- Keep the legacy iPad-on-Mac availability through native Mac launch and a verified transition. Whether to withdraw it afterwards is **not a decision that exists**: Pricing and Availability states that when the macOS version is approved, the iPhone/iPad app stops being available to Mac users. Apple withdraws it automatically at approval, so the transition instruction has to be in users' hands before the Mac version is approved, not before it is released. Two builds sharing `com.keevault.app` on one Mac is a known-bad configuration — see package 5. The default Mac artifact target is universal `arm64,x86_64`; an arm64-only exception requires an explicit product decision and matching support copy.
- The first native Mac version supports local files and WebDAV, not Dropbox or OneDrive. Dropbox/OneDrive caches are not converted into local vaults: a stale cache that looks like a vault is a data-loss shape, and the user's own synced folder is a live file the desktop client keeps current. Point them there instead, and never imply the account remains connected.
...
- [x] **1. Reconcile the release state model and skills.** Update `.agents/skills/release/SKILL.md`, `.agents/skills/release/xcode-cloud-setup.md`, `.agents/skills/release/gate-adjudication.md`, `.agents/skills/publish-app-store-version/SKILL.md`, and `ci_scripts/README.md`.
  - Replace every two-target bump and every reset-to-`1` instruction: `CURRENT_PROJECT_VERSION` increases globally across minor, major, patch, and respin releases on all four targets so Sparkle can compare direct builds safely.
  - Treat an RC as `{version, repo build, rc tag, SHA, iOS TestFlight build, Mac TestFlight build, direct CFBundleVersion}`; App Store Connect build numbers are platform-specific and must be matched back to the tag/SHA.
  - Add the macOS GitHub Actions gate, Mac KDBX gate, local Mac smoke suite, direct build, Mac soak, and two independent App Store submissions to the numbered workflow. Change first-launch release behavior from automatic/immediate to manual release; keep the rating. Move `v{version}` creation until after App Review code approval and the final go decision so it remains a record of code that actually shipped.
  - Done when a Cut → Respin → Ship → Patch walkthrough and `gate-adjudication.md` contain no `both targets`, build reset, single-TestFlight-build, automatic-release, or platform-incomplete gate assumptions.
...
- [ ] **2. Add Mac archive/TestFlight delivery to Xcode Cloud's `Tests (RC)` workflow.** Configure App Store Connect/Xcode Cloud and document the final action graph in `.agents/skills/release/xcode-cloud-setup.md`.
  ...
  - [x] App Store Connect is configured. App `6759309295` now carries the macOS platform — that action *is* the universal purchase, Apple offers no separate toggle, and a platform cannot be removed once added. **Tests (RC)** ends up with three actions started by one `rc/*` tag: **Archive - macOS** (scheme `KeeForgeMac`, Build For *Any Mac*, native rather than Mac Catalyst, prepared for App Store Connect), **Archive - iOS**, and **Test - iOS** (Required to Pass). The **TestFlight External Testing - iOS** post-action was deleted, so no platform auto-distributes to external testers any more and every external release is a manual decision. An empty **KeeForge Mac Test** external group exists for the native build. Both cloud secrets render redacted, confirming the Secret flag the earlier audit could not verify; **Restrict Editing** stays off.
  - [x] The setup candidate did its job and found three real problems, all now fixed. The macOS App Store version record was auto-created as **1.0** and is corrected to 1.15.0. The iOS upload was rejected — 90186 invalid pre-release train, 90062 version not higher than approved — because iOS 1.15.0 had already shipped and closed that train; the marketing version is now 1.16.0. And `Test - macOS` failed every test with `Could not launch "KeeForge"` / Runningboard error 5: Xcode Cloud's macOS VM will not spawn the sandboxed, signed Mac app, and since `KeeForgeMacTests` sets `TEST_HOST` to that app, the unit bundle never ran either. That action is removed — the settings that make this work on GitHub Actions are `xcodebuild` flags an Xcode Cloud test action cannot accept, and `Test - iOS` plus `macos-rc-tests.yml` already cover the same sources.
  - [x] `macos-rc-tests.yml` now runs on `macos-26`. Its first `rc/*` run failed in `actool`, which crashes compiling the `KeeForge.icon` Icon Composer bundle for macOS under the Xcode 26.3 that `macos-15` tops out at; `macos-26` carries the Xcode 26.6 that Xcode Cloud archives with. The workflow now also fails with a clear message if the runner offers anything older than 26.4, so a future image change cannot reintroduce the crash silently.
  - Setup candidate `rc/1.15.0-b3` was repo build 3 on all four targets with no product change, pushed only to prove that one tag starts the Xcode Cloud workflow and both GitHub Actions gates and that the archives reach their TestFlight lists. **The tag has since been deleted by the owner and no longer exists in the repository**, so repo build 3 is consumed but unanchored: what survives is the three check URLs below and the processed TestFlight build, not a ref that pins them to a SHA.
  - The tag started all three gates as designed, and their verdicts are recorded. Check URLs: [Xcode Cloud build 53](https://appstoreconnect.apple.com/teams/d9826238-924a-40e8-90dc-d1ae4bbb19dd/apps/6759309295/ci/builds/64184a97-fa23-474d-bf2a-13d624f99ba9/summary), [iOS 18 RC Tests](https://github.com/KeeForge/KeeForge/actions/runs/34055055898), [macOS RC Tests](https://github.com/KeeForge/KeeForge/actions/runs/34055055905). iOS 18 RC Tests passed. `Archive - macOS` succeeded and **macOS TestFlight received `1.15.0 (53)`, Complete and Ready to Submit** — the Mac delivery path is proven end to end. `Test - iOS` reported 1842/1855, its 9 failures the known secure-field keyboard-focus flake, which the GitHub iOS run passing over the same suites supports.
  - What the setup candidate did **not** prove is the iOS upload under the new app record: `Archive - iOS` failed on the closed 1.15.0 train, so no iOS build has reached TestFlight since the record became universal purchase and the post-action was deleted. Judged low risk and deliberately not re-run — iOS archiving worked on `rc/1.15.0-b1` and `-b2`, and the 1.16.0 bump reopens the train — so the first real candidate is where it gets confirmed.
...
- [x] **3. Automate the direct artifact and GitHub Release handoff.** Extend `ci_scripts/build_mac_direct.sh` or add a companion script under `ci_scripts/`; update its README in the same change.
  ...
  - Done when the offline fixture validates the exact zip, base-mismatch refusal, older-item preservation, duplicate refusal, local draft-download byte-check abstraction, and atomic publication against a matching existing base without network; the guarded handoff path resumes only an exact draft and verifies its asset, while final public URL verification and appcast publication remain separate; ordinary `xcodegen generate` is restored with a clean tracked worktree.

- [x] **4. Finish and prove the Sparkle trust chain.** The private EdDSA key stays in the login Keychain; export its recovery copy with Sparkle's `generate_keys -x` and store it in the KeeForge vault/off-machine, never in the repo or logs.
  - [x] User-attested prerequisite: the Sparkle EdDSA recovery-copy backup is complete. Secret contents and location are intentionally not recorded; the live trust-chain rehearsal remains required.
  ...
  - Artifact preflight is available as `ci_scripts/verify_mac_artifact.sh`; it fail-closes on missing sandbox/hardened runtime, wrong architectures, MAS Sparkle/feed/key content, or direct StoreKit/Sparkle/feed/key boundary drift, and prints no key value. The direct target now conditionally compiles out StoreKit sources and startup references; the live update/trust-chain rehearsal remains manual.
  - [x] Both channels are verified against real exported artifacts, not just source guards. Distribution-signed archives of the App Store and direct specs were built from the same tree and each run through the verifier: MAS passes with no Sparkle framework, XPC service, feed URL, or public key; direct passes with an HTTPS `SUFeedURL`, a present `SUPublicEDKey`, and no StoreKit. Two boundary defects were found this way and are the reason the check is now evidence rather than intent — a pre-fix direct artifact linked StoreKit, and the App Store build shipped a live `SUFeedURL`/`SUPublicEDKey` because `BuildConfig.local.xcconfig` sets them project-wide. `project.yml` now blanks both settings on `KeeForgeMac` and `project-direct.yml` inherits them back, so the safe default belongs to the App Store spec.
  - [x] The live rehearsal found that the direct channel could not install an update at all. Against a local HTTPS test feed the older build fetched the appcast, downloaded the zip and verified its EdDSA signature, then hung on "Updating KeeForge" forever: a sandboxed app cannot submit Sparkle's installer job itself, and the bootstrap look-up for `com.keevault.app-spks` was denied (`xpc_error=[159]`, then Sparkle's own `Failed to submit installer job` / sandboxing pointer). The direct spec now sets `SUEnableInstallerLauncherService` and selects `KeeForgeMac/KeeForgeMacDirect.entitlements`, which adds the `-spks`/`-spki` mach-lookup exceptions and nothing else; the App Store build keeps the base file and carries no temporary exception. `verify_mac_artifact.sh` now checks that boundary from the built artifacts in both directions. Nothing static could have caught this — every stage before the install succeeded.
  - [x] Both halves observed on real notarized artifacts, against a local HTTPS test feed (`https://localhost:8443`, mkcert leaf, loopback only; the production feed was never contacted). Older 1.15.0 (3) and newer 1.16.0 (4) were built fresh from the fixed tree, same key pair, `arm64,x86_64`, both `result=pass`.
    - **Update succeeds.** With the disposable vault unlocked and visible, `Check for Updates…` offered 1.16.0, downloaded it, verified the EdDSA signature, installed and relaunched, with no hardened-runtime or sandbox exception and no authorization prompt. Gatekeeper still reported `accepted / Notarized Developer ID` and the stapled ticket still validated afterwards.
    - **The relaunch boundary holds.** After the updater restart the vault was locked and required the master password again — a secure text field and `Unlock Database`, no unlocked session or decrypted content carried across — and the vault re-read identically once unlocked.
    - **An altered zip is rejected.** One bit was flipped with the file size left identical, so the appcast's `length` still matched and only the signature could catch it: Sparkle reported "The update is improperly signed and could not be validated", the older app stayed installed and running, and no successful installation was reported. Restoring the exact signed bytes reached Ready to Install again on the same feed and signature.
  - [x] The clean-Mac leg found a release blocker, and it is fixed. The direct zip that Gatekeeper accepted on the build machine was **rejected on a clean Mac** — "unsealed contents present in the root directory of an embedded framework". `ditto -c -k` stores extended attributes as inline AppleDouble `._name` entries; Apple's extractor consumes them, so Finder, `ditto -x -k` and Sparkle's own updater all produced a correct bundle and every build-machine check passed, but plain `unzip` and most third-party unarchivers materialize them as 201 real files inside the bundle, one in the root of `Sparkle.framework`. `build_mac_direct.sh` now packages with `--sequesterRsrc` and fails the build unless the finished zip, extracted with plain `unzip`, has no AppleDouble strays, is accepted by Gatekeeper, and still validates its staple. Verifying the exported `.app` could never have caught this: the damage exists only after a zip round trip. Re-tested on the clean Mac with the corrected packaging: downloaded afresh, `com.apple.quarantine` intact, launched without clearing it — **Gatekeeper accepted**. Step 6 is closed, and it is the only check that could have caught this: the update path never exercises a manual extraction, so the Sparkle rehearsal passed over the defect.
  - [ ] One leg remains open: the **GitHub Releases redirect**. The real enclosure URL redirects to `objects.githubusercontent.com` and Sparkle must follow that chain, which a local test feed cannot exercise. Half of it is already automated — `release_direct_artifact.sh verify-public-url` fetches the real URL unauthenticated with `curl -fsSL` and byte-verifies SHA-256 and size — so what is untested is specifically Sparkle's own sandboxed downloader following the cross-origin hop. It closes at the first real release, alongside package 2's iOS upload.
  - Evidence without secrets is recorded in the rehearsal's own notes; the private EdDSA key was never generated, exported, printed, or moved out of the login Keychain.

### Phase 2 — Move existing iPad-on-Mac users safely

The 2026-09-08 probe settled the design: the iPad-on-Mac app never stores a database inside its own containers. Databases live wherever the user saved them and are reached by a security-scoped bookmark; the App Group holds only `database-list.json` and a byte-identical AutoFill cache copy. There is nothing to rescue out of a container, so the transition is a one-time instruction, not a migration service.

- [x] **5. Run the production-state migration probe before writing migration code.** Observed on Apple silicon, macOS 26.6.2, App Store build `1.15.0 (52)`, against a locally built Apple Development-signed `KeeForgeMac`.
  - Creating a vault in the iPad build wrote `~/Documents/probe-local.kdbx` — outside every container — and stored an iOS-format bookmark (`book\x10`, 784 bytes). The App Group container is the unprefixed `~/Library/Group Containers/group.com.keevault.shared`, and its `databases/<uuid>.kdbx` matched the source file's SHA-256 exactly.
  - There is no `~/Library/Containers/com.keevault.app`. The iPad build gets a UUID-named container whose `Owners` records `validationCategory: app_store`.
  - A native build launched against that container listed the vault with its correct nickname and last-opened time, with no migration code. iOS bookmarks resolved under the native sandbox (both `.withSecurityScope` and plain) and the file read back matching bytes — promising, but measured in a test host that later proved unreliable, so treat bookmark transfer as unsettled. Nothing in the plan below depends on it.
  - **Blocker found.** With both apps installed under `com.keevault.app`, macOS binds the container to one signing `validationCategory` and silently denies the other. The native app showed "No Databases"; then so did the *iPad* app, with `database-list.json` and the cache still intact on disk. Entitlements, provisioning profile and signature were unchanged and valid throughout. No error is surfaced — the user simply sees an empty vault list where their vaults were. Recovery required deleting the group container, relaunching the iPad app to recreate it, then restoring the files from a copy.
  - Deliberately not established, because nothing now depends on it: stored composite key transfer, WebDAV reconnect, cloud cache recovery, quick-launch preservation, duplicate-reference identity, AutoFill provider contention.
  - Artifacts in `scratch/migration-probe/` (pre-probe container backup, seeded fixture, in-sandbox probe test); logs in `scratch/xcode-logs/*MigrationProbe*`. The probe test is not committed — it reads live production state.

- [x] **6. Ship the first-run transition note.** One sheet in the native Mac app, shown once, its shown/dismissed state in `UserDefaults.standard`. No migration service, no per-database result states, no retry machinery. It must not depend on reading the old list, so it is correct whether that list appears complete, stale, or empty.
  - Say the database files themselves are untouched, then: remove each old entry and add the file again from Finder. Deleting first is required — `DatabaseListStore.existingLocalReference` resolves the stored bookmark and compares paths, so re-adding a still-resolving reference is refused as a duplicate.
  - `DatabaseListStore.remove(id:)` deletes only the AutoFill cache, the backup directory, pending-upload markers and the stored composite key; it never deletes the user's file. Keep that property — the note depends on it.
  - Cloud entries need different wording: WebDAV reconnects the account; Dropbox and OneDrive are not in Mac v1, so the user opens their synced folder as an ordinary local file. Tell users to let the iPad app finish syncing before switching — that is the only remaining data-loss path.
  - Check whether an `isDocumentsResident` reference is reachable at all on iPad-on-Mac. If it is, detect it and say to export that one from the iPad app first, because its only copy is inside the container; if it is not, drop this case entirely.
  - Point AutoFill re-enable at System Settings → General → AutoFill & Passwords.
  - Shipped as `MacTransitionNoticeService` + `MacTransitionNoticeView`, presented once by `AppRootView` ahead of release notes and Quick Launch, both of which resume on the next launch. Verified on a real transition: the sheet appears with the iPad build's list behind it, dismisses, and does not return. All four steps are translated in every shipped locale; `LocalizationTests` passes on both platforms.
  - Correction to the first cut: the trigger cannot be a bare `fileExists` on `database-list.json`. In the fully wedged state the App Group is denied outright and `containerURL(forSecurityApplicationGroupIdentifier:)` returns nil, so there is no path to test and the sheet silently did not appear — observed on a real run before the fix. The service now treats an unresolvable container as its own trigger, which is sound because a fresh install always gets its group. The wedge is intermittent, so this specific path is covered by unit test rather than by a reproduction on demand.
  - `isDocumentsResident` is dropped rather than handled, on this evidence: the iPad-on-Mac container's `Data/Documents` is empty after a real seeding pass, `DatabaseCreationService` never writes there (creation goes through a save panel to a user-chosen location), and while the iOS `Info.plist` the iPad build carries does declare `UIFileSharingEnabled`, macOS exposes no Finder file-sharing surface for a Designed-for-iPad container. The state stays reachable on real iPhones and iPads, so `DocumentsVaultScanner` is unchanged; it is the Mac transition that cannot produce one.

- [ ] **7. Document the shared-identity risk, then verify minimally.** **Decided 2026-09-09: the direct build keeps `com.keevault.app`.** Separating the identifier was considered and rejected as not worth the profile, entitlement and App Group churn for a channel with no users yet. The accepted exposure, from the package 5 probe: no `.kdbx` file is damaged — the source file's SHA-256 held throughout, and saves fail closed — but the apps lose access to the shared container, so the reference list and its metadata (nicknames, quick launch, AutoFill selection, read-only, colour tags), the on-device backups, and any **unsynced Dropbox/OneDrive edits** can be lost if the container is deleted to recover. AutoFill silently returns nothing while wedged. The behavior was intermittent — the native build read the container fine twice before failing — and was only ever observed with a dev-signed build, so the rate on a real Developer ID build is unknown.
  - Because the failure presents as "No Databases" with no error, the risk that matters is the user's response, not the bug: tell people plainly that their files are safe and that nothing should be deleted or reinstalled. Worth considering before launch: distinguish "no databases yet" from "a list is present but unreadable" so the empty state can say which it is.
  - Minimum verification, and nothing beyond it: (a) a TestFlight Mac build installed over a seeded iPad app still lists its databases — folds into package 8; (b) the chosen direct-channel fix leaves the iPad app's own list intact; (c) remove-then-re-add restores a working vault — **done**, `DatabaseListStoreTests.testRemoveThenAddSameFileRestoresReferenceAndKeepsFile`, which also pins that removal leaves the file on disk.
  - Publish a short keeforge.com page carrying the same instruction. Do not withdraw the iPad app's Mac availability before the native app is live and verified.

### Phase 3 — Prepare the Mac App Store submission

- [ ] **8. Create the one-time App Store Connect Mac setup.** The account-level half is done — it landed during package 2's Xcode Cloud setup, since adding the platform is the prerequisite for a Mac archive to have anywhere to go. App `6759309295` carries the macOS platform, which *is* the universal purchase; there is no separate app record and no second bundle identity. The **KeeForge Mac Test** external group exists, and macOS TestFlight holds `1.15.0 (53)`, Complete.
  ...
  - Done when a processed throwaway Mac build can be installed through TestFlight by an account that owns KeeForge on iOS. Build 53 is processed but assigned to **no group**, so nothing has installed yet. Assigning it to the internal **KeeForge Test** group needs no Beta App Review and is the cheapest way to close this; that same install is also package 7's check (a) that a seeded iPad app's list survives a native install.

- [x] **9. Complete Mac listing and reviewer metadata.** Saved 2026-09-09 on the macOS version record, which was corrected from the auto-created 1.15.0 to **1.16.0**. Every text and reviewer field is written; the only remaining submission inputs are the RC build, the screenshots, and the release notes that come from the shipped version's changelog section.
  - All seven locales the Mac page exposes are filled — en-US (primary), zh-Hans, zh-Hant, fr, de, ru, es-ES, exactly the iOS set. Each locale's copy mirrors that locale's own shipped iOS description rather than being translated fresh, with every iOS-only claim corrected: Touch ID for Face ID, local files and WebDAV for iCloud/Dropbox/OneDrive, the System Settings AutoFill path for the QuickType bar, screen-capture blocking for screen-recording blur, and clipboard clearing for Universal Clipboard. Each description closes by saying plainly that Dropbox and OneDrive are not on Mac yet and to open the synced folder as a local file.
  - The inherited fields were the trap, not the empty ones. Promotional text and description were blank, but keywords, support URL, marketing URL and reviewer notes had all been pre-populated from iOS and were **wrong rather than missing**: the keywords advertised `onedrive` and `faceid`, and four locales carried a stale `github.com/crazytan/KeeForge` support URL. Mac keywords now read `…,touchid,…,webdav,…` and every locale points at `github.com/KeeForge/KeeForge`. zh-Hant was also missing the marketing URL the other six had.
  - Reviewer notes are Mac-specific — File ▸ Open Database…, `test.kdbx.zip` with `testpassword123`, AutoFill at System Settings → General → AutoFill & Passwords, no sign-in, WebDAV optional — and `test.kdbx.zip` is attached. Sign-in required is off; contact information is populated.
  - Not touched, and deliberately so: name, subtitle, categories and age rating are app-level and shared with the live iOS listing, so there is nothing Mac-specific to write and changing them would edit the iOS page. Age rating is kept with **Keep Existing Rating** in the submission flow. Export compliance cannot be observed until a build is attached, so `ITSAppUsesNonExemptEncryption = false` remains a plist declaration rather than a resolved record — and the historical France-related "No" is still not an answer to reuse. The privacy URL lives on the App Privacy page, not the version page, and is still unverified.
  - The exact saved copy for all seven locales is recorded in `.agents/skills/publish-app-store-version/mac-listing-metadata.md`, written back from ASC after saving so the file and the record agree.

### Phase 4 — Accept, soak, review, and ship one RC

- [ ] **10. Cut one RC and produce all three artifacts.** Follow the corrected `release` skill: bump all four targets, run `xcodegen generate`, run `ci_scripts/run_kdbx_compatibility_gate.sh` for iOS and again with `KDBX_COMPAT_SCHEME=KeeForgeMac`, commit, and push one immutable `rc/{version}-b{repoBuild}` tag.
  - Accept Xcode Cloud, iOS 18 GitHub Actions, and macOS GitHub Actions for that SHA; run `KeeForgeMacUITests/MacSmokeUITests` locally on the unlocked release Mac. Resolve/adjudicate failures before any external distribution.
  - Obtain/export the exact MAS `.app` from the accepted Xcode Cloud archive without rebuilding it. Run `ci_scripts/verify_mac_artifact.sh --channel mas --app <exact-mas-app> --architectures arm64,x86_64`, then run `ci_scripts/build_mac_direct.sh` from the same clean SHA and verify its exact exported app with the corresponding `--channel direct` command. Both must pass before external distribution or direct-artifact staging; a different architecture set requires an explicit product decision recorded in the manifest.
  - Stage—but do not publish—the signed zip/appcast item only after both artifact checks pass. Populate the release manifest with version, repo build, tag, SHA, both TestFlight build numbers, direct CFBundleVersion, SHA-256, Sparkle signature, notarization ID, symbols/archive locations, and all gate URLs.
  - Done when all three artifacts trace to the same SHA and each can be installed without rebuilding.

- [ ] **11. Produce and review the final Mac screenshots.** Use the exact RC from package 10 with an unlocked login session and Screen Recording permission for `KeeForgeMacUITests-Runner`.
  - Under the repository Xcode lock, run `TEST_RUNNER_SCREENSHOT_AUDIT=1 xcodebuild test ... -only-testing:KeeForgeMacUITests/MacScreenshotAuditUITests` with an `.xcresult`, export attachments to `build/screenshots-mac`, and treat any `00-skipped-captures` entry as a failure.
  - Run `ci_scripts/make_appstore_screenshots.py --platform mac`; it must produce the seven named 2880×1800 files in `build/appstore-mac`. Review every image at full size for real data, clipping, stale copy, unsupported cloud claims, wrong version, and accidental desktop/background capture before upload.
  - Done when the seven uploaded screenshots match the exact RC and App Store Connect reports no missing Mac display-size assets.

- [ ] **12. Complete the release-candidate Mac acceptance matrix.** Daily-drive both channels; because they share a bundle ID, test MAS and direct sequentially or on separate clean Macs, not side by side.
  - Cover WebDAV reconnect/relaunch; local/external/key-file vaults; migration; AutoFill in Safari, Chromium, and native fields; verification codes; passkeys; cancel/relock; non-Touch-ID reveal authentication; screen lock/sleep/screensaver; clipboard clearing; capture blocking; VoiceOver; keyboard-only use; Increase Contrast; and Reduce Transparency.
  - Verify channel boundaries (`MAS: no Sparkle + StoreKit tip jar`; `direct: Sparkle + no StoreKit`), receipts, sandbox/App Group/Keychain/AutoFill entitlements, and clean uninstall/reinstall behavior.
  - The package-10 artifact checks must run on both exact exported apps. They must pass with universal `arm64 x86_64` for macOS 15 unless an explicit product decision changes the architecture set; if changed, record the Intel-support/product-page decision before submission rather than silently shipping different support matrices.

- [ ] **13. Soak the exact RC in parallel and make one go/no-go decision.** After all cloud and local gates and both exact-artifact checks are accepted, obtain a separate explicit action-time confirmation immediately before manually distributing each exact iOS and Mac build to its external TestFlight group; test the staged direct artifact on clean Apple-silicon and Intel hardware/VMs plus the full Sparkle cycle.
  - For each App Store build record distribution time, unique installs, new crash signatures, and P0/P1 feedback; target 48 hours, 5 installs, zero new crash signatures, and no open P0/P1 reports. Record direct installs/update results separately because it has no TestFlight metrics.
  - Any code change increments the shared build number, creates a new RC tag, rebuilds all three channels, and restarts their soak clocks. Metadata-only corrections do not require a binary respin.
  - iOS TestFlight readiness does not block Mac preparation, but do not promote any channel until both App Store builds and the direct zip are the same accepted RC and any short soak signal is explicitly accepted.

- [ ] **14. Stage and hold both App Store versions.** After the go decision, use `publish-app-store-version` once per platform to attach the exact soaked iOS and Mac TestFlight build numbers; stage each platform independently through **Ready for Review**, then stop. The immutable RC tag/SHA is the handoff, and `v{version}` does not exist yet.
  - Save localized release notes, screenshots, export compliance, fixture, reviewer instructions, and manual-release settings separately; do not click **Submit for Review** during staging. Immediately before each platform's submit click, require a separate explicit action-time user confirmation; a confirmation for iOS does not authorize macOS.
  - For the first build of each marketing version/platform, require separate explicit action-time confirmation immediately before Beta App Review and immediately before external tester distribution. Keep an approved platform held until the coordinated release decision; never substitute the newest unsoaked build.
  - If Apple requests metadata only, correct that platform's record. If Apple requires a code change, return to a shared respin.
...
- The read-only badge in a database's toolbar is no longer a second lock icon beside the Lock button. It is now a reading-glasses badge, sized like the buttons around it, and tapping it says why editing is off.
...
- Entries and groups can now be moved to another group (#80). Long-press an entry or group and choose "Move to Group" to pick a destination; the picker hides the recycle bin and (for a group) its own subtree, and moves sync correctly as KDBX location changes so other KeePass apps and the merge engine see them.
...
- When the iPad app runs on a Mac ("Designed for iPad"), Settings is reachable again: the app menu's Settings… item (⌘,) now opens KeeForge's own Settings instead of a generic compatibility window, and a Settings gear sits in the detail column's toolbar whether or not a database is unlocked (#69).
...
- A cloud or WebDAV database whose server doesn't answer now opens from the local cached copy after about ten seconds instead of waiting out the full network timeout (#95). The open shows the existing "Using the cached copy offline." notice; edits can't be saved to the server until it is reachable again. A WebDAV server that answers 502/503/504 (a gateway that can't reach its backend, or maintenance) takes the same calm cached-copy path instead of surfacing a server-error message.
...
- Reworked the release process around a `release/{major}.{minor}` branch and the public TestFlight channel. Candidate builds are archived from `rc/{version}-b{repoBuild}` tags, soaked with external testers, and the exact soaked build is the one submitted to the App Store — `v{version}` now records what shipped instead of triggering a fresh archive. Adds per-build build numbers, measured-and-reported soak signals (48h for a minor release, 24h for a patch), release-branch backports as real merges into `main`, `release/**` PR gating and branch rules, and `ITSAppUsesNonExemptEncryption` in `Info.plist` so ASC can resolve the build declaration from metadata; separate legal questions still require verification.
