...
- Copy database files straight from a computer: KeeForge's Documents folder now appears in Finder (and iTunes/Windows File Sharing) when the device is plugged in, and any `.kdbx` file dropped there shows up in the database list automatically (#63). Replacing a file with a newer copy of the same name keeps the existing entry — nickname, key file, and settings included — instead of creating a duplicate. If a file is deleted from the folder instead, its list entry says so and the open screen offers a one-tap "Remove from List"; the entry is never removed automatically, so dropping the file back in restores it.

- Download a website's icon straight into the database. The icon chooser has a "Download Website Icon" button that fetches the site's favicon, stores it in the vault as a custom icon and applies it to the entry — so it shows up on every device the database syncs to, and in other KeePass apps too, not just on the phone that downloaded it. The image is re-encoded to a bounded size first, so a site serving something huge cannot inflate your database, and downloading the same icon for several entries stores it once. Available whenever the entry has a website address; the button explains itself when there is nothing to download.
...
- An edit made while a save was still uploading is no longer lost. Saving a change to a slow destination (a cloud database on a weak connection) left a window where a second edit — picking an icon while the previous pick was being written, say — was silently dropped once the first save finished. The in-flight save now notices the newer change and writes it too, and the icon chooser's grids wait out an in-progress icon download instead of accepting a pick the save would have discarded.
