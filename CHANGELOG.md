...
- [ ] **UX sweep and acceptance test.**
  ...
  - [x] Second manual UI sweep, fixes:
    ...
    - The New Database sheet renders as a grouped, inset form like the other editors: its `Form` was missing `.macGroupedForm()`, so macOS fell back to the borderless columns style with sections running edge to edge.
