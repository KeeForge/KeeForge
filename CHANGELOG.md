...
- [ ] **2. Add Mac archive/TestFlight delivery to Xcode Cloud's `Tests (RC)` workflow.** Configure App Store Connect/Xcode Cloud and document the final action graph in `.agents/skills/release/xcode-cloud-setup.md`.
  ...
  - Setup candidate `rc/1.15.0-b3` exists for exactly this purpose: repo build 3 on all four targets, no product change, pushed only to prove that one tag starts the Xcode Cloud workflow and both GitHub Actions gates, and that the archives reach their TestFlight lists. It is not a release candidate.
  ...
  - Done when a non-shipping setup RC tag (kept permanently) has both its iOS and Mac builds reach their TestFlight lists with the three check URLs recorded. `rc/1.15.0-b3` is that tag and satisfies everything except the iOS half above; do not use it for a release.
...
- [ ] **5. Run the production-state migration probe before writing migration code.** On an Apple-silicon Mac, install the current App Store "Designed for iPad" build and seed local, external-file, key-file, stored-key, WebDAV, Dropbox, OneDrive, multi-database, quick-launch, and AutoFill cases.
  ...
  - **Probe status (2026-09-02):** the App Store build is installed at `/Applications/KeeForge.app` (version `1.15.0`, build `52`, `iphoneos`, arm64) but has not been launched or seeded; all production fixture outcomes remain unobserved. See `KeeForgeMac/README.md`.
