...
- [ ] **7. Decide the direct channel's bundle identity, then verify minimally.** The blocker is a direct-download problem: that build coexists with the iPad app, whereas the Mac App Store build replaces it. Either give the direct build its own bundle identifier, or make its first launch refuse to run while the iPad app is installed.
  - Minimum verification, and nothing beyond it: (a) a TestFlight Mac build installed over a seeded iPad app still lists its databases — folds into package 8; (b) the chosen direct-channel fix leaves the iPad app's own list intact; (c) remove-then-re-add restores a working vault, covered by one shared test in `KeeForgeTests/`.
