...
- Keep the legacy iPad-on-Mac availability through native Mac launch and a verified transition; decide whether to withdraw it only afterwards. Two builds sharing `com.keevault.app` on one Mac is a known-bad configuration — see package 5. The default Mac artifact target is universal `arm64,x86_64`; an arm64-only exception requires an explicit product decision and matching support copy.
...
- [ ] **8. Create the one-time App Store Connect Mac setup.** With explicit account-level confirmation, add macOS to the existing KeeForge app record and, if the owner confirms the recommended product choice, enable universal purchase; do not create a separate app record or bundle identity. Until this is done, universal purchase is intended/pending, not current state.
  - Create the Mac TestFlight external group and confirm the native build recognizes ownership of the iOS app. Keep the iPad app available on Mac during migration/launch preparation.
  - Recommended default: configure manual release for both platform records for this coordinated launch. Phased release for macOS is optional and not yet decided; use it only after an explicit owner choice, otherwise manual timing is the rollout control.
  - Done when a processed throwaway Mac build can be installed through TestFlight by an account that owns KeeForge on iOS.
...
- [ ] **9. Complete Mac listing and reviewer metadata.** In App Store Connect, prepare the Mac-specific promotional text, description, keywords, support/privacy URLs, category/age-rating fields, and release notes in every localization shown on the Mac version page.
  - Say that v1 supports local files and WebDAV but not Dropbox/OneDrive. Reviewer notes must explain how to open the attached `test.kdbx.zip` (`testpassword123`) and enable AutoFill in System Settings → General → AutoFill & Passwords.
  - Verify contact information, attachment visibility, `ITSAppUsesNonExemptEncryption = false` resolution, live public availability including France, rating retention, the chosen manual/phased release setting, and the absence of sign-in requirements beyond an optional WebDAV test. The build declaration does not by itself resolve any separate ASC legal/documentation request.
  - Local status (2026-09-02, read-only): the paste-ready template is in `.agents/skills/publish-app-store-version/mac-listing-metadata.md`. Verified from the existing iOS record: locales are en-US (primary), zh-Hans, zh-Hant, fr, de, ru, and es-ES; copyright is `© 2026 Jia Tan`; Support URL is `https://github.com/KeeForge/KeeForge?tab=readme-ov-file#support`; Marketing URL is `https://keeforge.com/`; categories are Productivity / Utilities; age rating is 4+ (172 regions, with Brazil/Korea/Vietnam variants); and public availability includes France. The prior export-compliance answer recorded as “No” for France is ambiguous against that live availability and must not be reused or inferred without reading the exact accepted iOS Build Metadata and current question text and obtaining action-time owner confirmation. The Mac page still must be created, verified, and saved; package 9 remains unchecked.
