...
- [ ] **10. Cut one RC and produce all three artifacts.** Follow the corrected `release` skill: bump all four targets, run `xcodegen generate`, run `ci_scripts/run_kdbx_compatibility_gate.sh` for iOS and again with `KDBX_COMPAT_SCHEME=KeeForgeMac`, commit, and push one immutable `rc/{version}-b{repoBuild}` tag.
  ...
  - After the MAS archive is cut, run `ci_scripts/build_mac_direct.sh` from the same clean SHA and stage—but do not publish—the signed zip/appcast item. Populate the release manifest with version, repo build, tag, SHA, both TestFlight build numbers, direct CFBundleVersion, SHA-256, Sparkle signature, notarization ID, symbols/archive locations, and all gate URLs.
...
- [ ] **12. Complete the release-candidate Mac acceptance matrix.** Daily-drive both channels; because they share a bundle ID, test MAS and direct sequentially or on separate clean Macs, not side by side.
  ...
  - Run `lipo -archs` on both exported apps. Prefer universal `arm64 x86_64` for macOS 15; if either channel is single-architecture, stop and make the Intel-support/product-page decision before submission rather than silently shipping different support matrices.

- [ ] **13. Soak the exact RC in parallel and make one go/no-go decision.** Distribute the exact iOS and Mac builds to their external TestFlight groups at the same time; test the staged direct artifact on clean Apple-silicon and Intel hardware/VMs plus the full Sparkle cycle.
...
- [ ] **14. Submit and hold both App Store versions.** After the go decision, use `publish-app-store-version` once per platform to attach the exact soaked iOS and Mac TestFlight build numbers; the immutable RC tag/SHA is the handoff, and `v{version}` does not exist yet.
  - Save localized release notes, screenshots, export compliance, fixture, reviewer instructions, and manual-release settings separately; submit iOS and macOS for review separately and track their states independently.
  - If Apple requests metadata only, correct that platform's record. If Apple requires a code change, return to a shared respin. Keep an approved platform held until the coordinated release decision; never substitute the newest unsoaked build.
