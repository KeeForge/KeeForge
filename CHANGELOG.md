...
- App Store Connect has separate iOS and macOS version records, builds, screenshots, metadata, and review submissions. Hold both with manual release for the first coordinated launch.
...
- Use GitHub Releases for immutable direct-download zip hosting. R2 is not required for v1. The production appcast remains `https://keeforge.com/appcast.xml` and is published only after its zip is final and reachable.
...
- [ ] **5. Run the production-state migration probe before writing migration code.** On an Apple-silicon Mac, install the current App Store "Designed for iPad" build and seed local, external-file, key-file, stored-key, WebDAV, Dropbox, OneDrive, multi-database, quick-launch, and AutoFill cases.
  - Inspect the shared App Group `group.com.keevault.shared`: `database-list.json`, `databases/`, and `cloud-cache/`; then install a native development build without deleting the iPad app and record which references, encrypted caches, and `com.keevault.sharedkeychain` items it can actually read.
...
- [ ] **7. Gate migration and document recovery.** Add focused shared tests under `KeeForgeTests/` so they also run as `KeeForgeMacTests`; add only the smallest Mac UI smoke needed for first-launch/result/retry behavior.
  ...
  - Run the relevant migration tests, `AppGroupGuardrailTests`, `SecurityScopedBookmarkManagerTests`, both KDBX compatibility gates if cache-copy semantics change, and a manual transition with the production iPad app still installed.
  - Publish keeforge.com instructions covering backup/export first, each result state, WebDAV reconnection, the Dropbox/OneDrive limitation, AutoFill re-enable, retry/manual export, and the guarantee that the old app's data remains until the user deletes it. Do not withdraw the iPad app's Mac availability before the native app is live and verified.

### Phase 3 — Prepare the Mac App Store submission

- [ ] **8. Create the one-time App Store Connect Mac setup.** With explicit account-level confirmation, add macOS to the existing KeeForge app record and enable universal purchase; do not create a separate app record or bundle identity.
  ...
  - Configure manual release for both platform records for this coordinated launch. Use phased release for macOS only if App Store Connect offers it for an initial Mac version; otherwise manual timing is the rollout control.
