...
- [ ] **2. Add Mac archive/TestFlight delivery to Xcode Cloud's `Tests (RC)` workflow.** Configure App Store Connect/Xcode Cloud and document the final action graph in `.agents/skills/release/xcode-cloud-setup.md`.
  ...
  - Two things the setup surfaced and left open: ASC auto-created the macOS version record as **1.0**, which does not match the repo's `MARKETING_VERSION` and has to be corrected before a Mac build can attach to an App Store version (TestFlight is unaffected); and `Test - macOS` inherits the `KeeForgeMac` scheme's test action, which includes `KeeForgeMacUITests` — a suite this repo otherwise treats as local-only because it needs a real login session. `Test - iOS` already runs `KeeForgeUITests` the same way on Xcode Cloud, so this is likely fine, but the setup RC tag is what will prove it.
