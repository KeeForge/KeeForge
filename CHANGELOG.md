...
- [ ] **10. Cut one RC and produce all three artifacts.** Immutable `rc/1.16.0-b48` at `a48d6b1` was rejected and never distributed: its Xcode Cloud creation failures were reproduced, and the green iOS 18 GitHub Actions verdict hid canonical XCTest UI failures. Immutable `rc/1.16.0-b49` at `eacbc3bb8a1dc8116e339dda1be31534b5bc3310` is the replacement. Xcode Cloud build 55 archived/uploaded both iOS and macOS `1.16.0 (55)`, each **Complete** and **Validated**, but its iOS Xcode Cloud and GitHub iOS 18 test gates are still running, so the candidate is not accepted.
  ...
  - The direct b49 artifact is notarization-accepted, stapled, Gatekeeper-accepted, and plain-`unzip` verified. Final Sparkle Keychain signing is waiting for the owner. Neither b49 platform build has been added to an external group or submitted for beta review.
