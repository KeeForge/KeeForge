...
Keyboard-first pass (2026-08-22):
- **The vault is navigable from the keyboard.** All three Mac columns were hand-rolled buttons with a manual highlight, so there was no arrow-key movement, no type-select, no focus ring, and no double-click-to-open anywhere in the app a Mac user actually lives. The sidebar (group tree + tags) and the entries column are now native `List(selection:)` bound to the selection the view model already owned, so all of that comes from AppKit. Double-click on an entry opens its editor, and Return does the same. Search results and tag results still use the shared iOS entry list, so they keep click-only rows for now.
- **The menu bar covers the app.** New Group (⇧⌘N), Open Database… (⌘O — previously reachable only from the database list's file importer, which is off screen while a vault is open), Edit Entry (⌘E), Delete (⌘⌫), Copy URL (⇧⌘U), and Copy Verification Code (⇧⌘T) join the existing commands; Copy URL resolves KeePass `{REF:…}` field references the same way Copy Username and Copy Password do. Menu-bar Delete raises the *shared* confirmation the row context menus use — it cannot delete silently — and every item is disabled when it cannot apply.
- **A real About panel and a real Help menu.** `About KeeForge` opened nothing (the About content was buried in the Settings window's fourth tab) and the Help menu shipped a "KeeForge Help" item pointing at nothing.
- **Closing the last window now locks the vault.** Plain ⌘W closes the only window without quitting, and the session lives in app-level state, so an unlocked vault sat decrypted in memory with no window and no File ▸ New Window to get it back — only a Dock click. `MacLockMonitor` now treats the last UI-hosting window closing as a lock trigger like screen lock or sleep; unsaved edits still go through the usual discard prompt rather than being dropped.
- **Tooltips on icon-only controls.** Copy, reveal, regenerate, generate-password, the entry icon, key-file clear, and the database-list toolbar had accessibility labels but no hover explanation.
- **Notes render properly.** The Mac note view was an interim plain `Text` that could not size itself to the column; it is now a selectable `NSTextView` with width-aware height and clickable URLs and email addresses.

Visual/UX sweep (2026-08-22), from the `MacScreenshotAuditUITests` screenshot audit in both appearances:
- **The selected entry row's icon was invisible.** `FaviconView`'s standard-icon fallback draws with `.tint`, which no foreground style or list selection overrides, so on the accent-filled selection it rendered accent-on-accent at 1.34:1. It now reads `backgroundProminence`, so the glyph turns white exactly when the row's fill is the accent color and keeps its accent tint everywhere else — including a selected row in an unfocused list, where the fill is grey (4.64:1).
- **The macOS entry and group editors were laid out wrong in three ways at once.** Every field was labelled twice — iOS treats a `TextField`'s title as placeholder text and drops it once there is a value, while macOS renders it as a second, right-aligned label beside the row's own caption. Neither editor set `.formStyle(.grouped)`, so macOS fell back to its default `.columns` style: no insets, rows running edge to edge, and the sheet's title clipped in half by its own top edge. And inside a grouped form macOS draws text fields borderless, leaving each row a caption over empty space with nothing showing where to type. Three macOS-only helpers (`macLabelsHidden`, `macGroupedForm`, `macFormFieldStyle`) fix all three without touching iOS, where list rows are borderless by convention and the placeholder is the label.
- **The four other macOS sheet forms overflowed the same way the editors did.** `DatabaseDetailsView`, `MasterKeyChangeView`, `WebDAVConnectView`, and `FeedbackComposerView` all set no form style either, so macOS gave them the non-scrolling `.columns` layout: the details sheet lost its title and Identity section off the top edge and everything from Cloud Sync down off the bottom, and the master-key screen's key-file section fell off the bottom of the details sheet it is pushed into — rows no click could reach, only Tab. All four now take `macGroupedForm()`, with `macFormFieldStyle()` on their text fields (a grouped form draws them borderless) and `macLabelsHidden()` where a `LabeledContent` caption or a section header already names the field. The WebDAV sheet was not clipping, because it sizes itself to its content, but it carried the same double label and edge-to-edge rows.
- The screenshot-audit harness was capturing the desktop instead of app windows: it snapped `windows.firstMatch` (the main window, mislabelled as "settings" when the settings window had not opened), fired ⌘-shortcuts without checking the app had focus, and reported nothing when a capture silently failed. It now addresses secondary surfaces by content, refuses to type a shortcut the app would not receive, and records every skipped capture. The desktop leak itself was the app's own doing: `blockScreenCapture` defaults on, so `sharingType = .none` excluded KeeForge's windows from the capture composite and a screen-region capture returned whatever sat behind them — the audit now launches with that protection off and captures each window individually. It also covers the AutoFill settings tab and the 900pt minimum-width layout, neither of which it walked before.
...
- [ ] Slice 07 — distribution. The in-repo half is done: a channel seam (`DistributionChannel`), the `project-direct.yml` overlay spec that adds Sparkle and defines `KEEFORGE_DIRECT_DOWNLOAD`, the tip-jar-vs-Sponsors swap, StoreKit suppressed in direct builds, a Check for Updates menu item, `SUFeedURL`/`SUPublicEDKey` wired to build settings, and `ci_scripts/build_mac_direct.sh` (archive → export → entitlement check → notarize → staple → Gatekeeper → appcast zip). Verified both ways: the App Store build links no Sparkle, the direct build does. Still needs the human half — App Store Connect Mac platform + universal purchase, TestFlight, a Developer ID certificate, hand-made Developer ID provisioning profiles for `com.keevault.app` and `com.keevault.app.autofill` (xcodebuild cannot auto-create them), a `notarytool` keychain profile, an EdDSA key pair, and HTTPS appcast hosting. None of it has been exercised end to end, because none of those credentials exist yet.
- [ ] Keyboard navigation in search results and the tag browser, which still render the shared iOS entry list.
- [ ] Remaining Mac UX polish: macOS sheet sizing and hover feedback on the workspace and database-list rows.
