...
- [x] **Close feature gaps with the latest iOS wherever applicable.**
  - [x] AutoFill provider does not appear in System Settings → AutoFill & Passwords despite correct registration. **Fixed:** the Mac app target was missing `com.apple.developer.authentication-services.autofill-credential-provider`. The extension had it and the iOS app target had it; only `KeeForgeMac/KeeForgeMac.entitlements` did not. macOS lists a credential provider only when the *containing app* claims that entitlement too, so `pluginkit` registered the appex and the pane stayed empty. The App ID already carries the capability and the Developer ID profile already authorizes it, so the entitlements file was the whole fix. Details in `KeeForgeMac/README.md`.
  - [x] Raise the minimum to macOS 15 and advertise one-time-code AutoFill. System Settings now lists KeeForge for passkeys, passwords, and codes, and KeeForge Settings can ask macOS to enable the provider directly.
  - [x] Multi-database AutoFill suggestions cannot aggregate on macOS. **Platform limit, now handled deliberately:** `ASCredentialIdentityStoreState.supportsIncrementalUpdates` is false there, so `saveCredentialIdentities` means "pass all credential identities", `removeCredentialIdentities` is unsupported, and store enumeration returns nothing attributable. The enumerate-and-diff aggregation built for iOS therefore left Safari suggesting from whichever database happened to be unlocked last, and every targeted removal silently did nothing — a disabled or deleted database kept its suggestions. `CredentialIdentityStoreManager` now reads the capability before it writes: without incremental updates a refresh is one whole-store replace and a targeted removal clears the store, so the most recently unlocked database owns the suggestions and nothing goes stale. Other databases stay reachable through the extension's database switcher, and macOS AutoFill settings states the limit whenever more than one database is selected. The gate is the runtime flag, not `#if os(macOS)`.
  - [x] Dropbox/OneDrive stay hidden on macOS. **Decided rather than deferred:** the first macOS release ships WebDAV only. The OAuth paths are implemented and unit-tested on iOS but have never been validated end to end on a Mac, and unhiding them is a later release's call. Both providers are now excluded from the Mac build outright rather than compiled and hidden, so the `SwiftyDropbox` and `MSAL` packages come off the `KeeForgeMac` target — MSAL alone was a 7.4 MB embedded framework that shipped whether or not a symbol was called. The Mac app also stops declaring the `db-*`/`msauth.*` OAuth callback schemes and the `DropboxAppKey`/`OneDriveClientID`/`OneDriveRedirectURI` keys it can no longer service.
...
- [ ] **UX sweep and acceptance test.**
  ...
  - [x] Second manual UI sweep, fixes:
    ...
    - The AutoFill panel is no longer a dead end when nothing matches. Its Cancel button, database switcher, and search field all lived in a SwiftUI `.toolbar`, which macOS routes to the window's `NSToolbar` — and the system credential-provider window the shell is hosted in has none, so all three were dropped and the panel rendered as a bare list with no way out, blocking the calling app until it was quit. The picker now draws its own title, search field, switcher, and Cancel (also bound to Escape) inside the view. The same slip left the passkey creator with neither Cancel nor Save Passkey, so macOS passkey registration could not be completed at all; it gains the same in-view bars, as does the no-databases-for-AutoFill empty state.
    - An attachment's Quick Look preview no longer lingers over another entry or sheet after you switch away from it.
    - The read-only badge is drawn once, at the size of the toolbar buttons around it. A bare `Image` in a toolbar keeps body size while the buttons beside it get the toolbar's larger glyph metrics, so it read as an undersized stray lock next to the Lock button — and because the entry detail's own toolbar merges into the same window bar, selecting an entry put a second one at the far end. It is a `Button` now (`ReadOnlyIndicator`), showing reading glasses that explain themselves when clicked, and the entry detail leaves the statement to the workspace toolbar on macOS.
    - The New Database sheet renders as a grouped, inset form like the other editors: its `Form` was missing `.macGroupedForm()`, so macOS fell back to the borderless columns style with sections running edge to edge. An audit for the same slip caught two more sheets with a bare `Form` — the password generator and New Group — now grouped the same way.
    ...
    - Clicking an entry now shows it immediately. The row carried two plain tap gestures — a double-click to open the editor and a single click to move the selection — and SwiftUI composes those exclusively, so every single click waited out `NSEvent.doubleClickInterval` (half a second by default) before the selection could move and the detail pane could render. The double-click is a simultaneous gesture attached after the single tap now, which measures 3ms instead of 353ms. `MacSmokeUITests` gained a double-click test so both halves stay covered.
  - [x] Entry version history now uses a Mac layout: the stored versions on the left, the selected version's fields on the right, and Restore/Done in one action bar. Opening a version used to replace the whole sheet, which left it with no Done button, clipped it at the bottom, and scrolled its first field out of sight. Supersedes the earlier fix that moved Restore into the version's content.
  - [ ] Manual QA matrix: cloud sign-in with relaunch token survival; AutoFill across Safari/Chromium/native password and verification-code fields and webauthn.io passkeys; cancel-everywhere relock; reveal-auth on a non-Touch-ID Mac; ⇧⌘4 capture blocking on macOS 26.
...
- [ ] **Release process readiness for both channels (MAS and direct).** The in-repo half of slice 07 is done — channel seam, `project-direct.yml` overlay, tip-jar-vs-Sponsors swap, StoreKit suppressed in direct builds, and `ci_scripts/build_mac_direct.sh` (archive → export → entitlement check → notarize → staple → Gatekeeper → appcast zip). The human half is untouched:
  ...
  - [x] Direct: `notarytool` keychain profile stored and exercised — the first notarized build was accepted, stapled, and passes Gatekeeper as `source=Notarized Developer ID`. The Developer ID certificate is in place, and the provisioning profiles turned out not to be a manual step: `xcodebuild archive` / `-exportArchive` with `-allowProvisioningUpdates` creates the profiles for `com.keevault.app` and `com.keevault.app.autofill` on demand, and the exported build is Developer ID signed and hardened on both bundles. `ci_scripts/build_mac_direct.sh` passes the flag.
  ...
  - [ ] Decide what happens to the iOS app on Apple Silicon Macs. Previously settled as **withdraw Mac availability for the iOS app**, on the reasoning that one bundle had to own `com.keevault.app` for Mac AutoFill to work at all. That reasoning is gone — the AutoFill bug was a missing app entitlement (above), and the native provider appears with the iOS app untouched — so the decision is re-opened on its remaining merits, which are presentation rather than function. Either way, users with databases and bookmarks in the iOS container need a migration story; the steps are worked out in `KeeForgeMac/README.md` and still need user-facing copy.
...
- [ ] **Sparkle readiness and test** (direct channel only).
  - [ ] EdDSA key pair generated; the private key lives in the login keychain and the public half is pinned in `BuildConfig.local.xcconfig`. **Still open: the off-machine backup.** Export with `generate_keys -x` and store it in the vault — losing it strands every direct install with no way to update.
  - [ ] `SUFeedURL`/`SUPublicEDKey` now point at real values — `https://keeforge.com/appcast.xml` and the generated key, both verified in the shipped Info.plist. **Still open: hosting the appcast** at that URL plus the `downloads/` enclosure path. (The release script fails the build if the feed is not HTTPS or the key is empty, and now signs the payload and prints the appcast `<item>` itself.)
