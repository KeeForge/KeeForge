...
- [x] **10. Cut one RC and produce all three artifacts.** Accepted candidate: immutable `rc/1.16.0-b51` at `c5f496942b56156cd7d79ed7851c74353822a7c0`. It fixes an initial-focus race: selecting a database could leave typing and Escape in the sidebar instead of the password field. The fix passed 12 focused unit tests and four Mac UI checks before the cut; RC51 then passed all 15 local Mac smoke tests.
  - Fresh RC51 iOS and Mac KDBX gates each passed 19 tests with zero failures/skips and 33 KeePassXC artifact checks. Their tested tree matches the immutable RC. GitHub macOS passed 1,707 tests with 22 expected skips. Xcode Cloud 57 produced processed, validated iOS and macOS `1.16.0 (57)`; both exact exported packages and the notarized, Sparkle-signed direct `1.16.0 (51)` ZIP passed verification. All four failing Xcode Cloud test/device pairs passed exact local iOS 26.5 reproductions; all three iOS 18 failures passed exact local iOS 18.6 reproductions. Canonical audits found no non-test failures, all gates are accepted, and the distribution manifest validates. Both TestFlight builds were submitted to their respective public groups: iOS is **Testing**, macOS is **Waiting for Review**. The exact direct ZIP and complete unpublished appcast are staged for owner testing; their hashes, signature, baseline feed and future GitHub Release asset URL are recorded in the RC51 handoff.
  - The preceding RC50 produced iOS and macOS TestFlight `1.16.0 (56)` and a signed, notarized direct ZIP. The owner authorized TestFlight first while away; iOS later reached **Testing** and macOS **Approved**. Its owner-present Mac smoke then found the focus defect (13 passed, one failed), which reproduced in isolation. Preserve those exact artifacts and beta records; its failed Mac gate is not accepted or inherited by RC51.
  - RC48 and RC49 were never distributed. Keep their immutable tags and evidence. Stage the current direct ZIP and unpublished appcast only after all current gates and both exact exported Mac artifact checks are accepted; production publication and soak remain separate steps.
...
- The key icon in the AutoFill bar above the keyboard now opens KeeForge so you can search for and pick a credential (#129). It used to fill the suggested one straight away, which left no way to reach another entry. Tapping the suggestion itself still fills it directly.
