...
- [ ] **10. Cut one RC and produce all three artifacts.** Current candidate: immutable `rc/1.16.0-b51` at `c5f496942b56156cd7d79ed7851c74353822a7c0`. It fixes an initial-focus race: selecting a database could leave typing and Escape in the sidebar instead of the password field. The fix passed 12 focused unit tests and four Mac UI checks before the cut; the full smoke and hosted gates must pass for RC51.
  - Fresh RC51 iOS and Mac KDBX gates each passed 19 tests with zero failures/skips and 33 KeePassXC artifact checks. Their tested tree matches the immutable RC. Exact evidence is recorded in the RC51 manifest; archive and distribution work is underway.
  - The preceding RC50 produced iOS and macOS TestFlight `1.16.0 (56)` and a signed, notarized direct ZIP. The owner authorized TestFlight first while away; iOS later reached **Testing** and macOS **In Review**. Its owner-present Mac smoke then found the focus defect (13 passed, one failed), which reproduced in isolation. Preserve those exact artifacts and beta records; its failed Mac gate is not accepted or inherited by RC51.
