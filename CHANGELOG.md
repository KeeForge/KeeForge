...
- [ ] **3. Automate the direct artifact and GitHub Release handoff.** Extend `ci_scripts/build_mac_direct.sh` or add a companion script under `ci_scripts/`; update its README in the same change.
  ...
  - Generate a complete appcast item while preserving older items, verify the remote zip's SHA-256/signature/size, and make appcast publication a separate final command so an incomplete release cannot be advertised.
  - Done when a dry run can stop at a draft GitHub Release with a verified downloadable zip and an unpublished appcast change, and ordinary `xcodegen generate` has been restored with a clean tracked worktree.
