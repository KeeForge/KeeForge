...
- [ ] **2. Add Mac archive/TestFlight delivery to Xcode Cloud's `Tests (RC)` workflow.** Configure App Store Connect/Xcode Cloud and document the final action graph in `.agents/skills/release/xcode-cloud-setup.md`.
  ...
  - [x] App Store Connect is configured (2026-09-06). App `6759309295` now carries the macOS platform — that action *is* the universal purchase, Apple offers no separate toggle, and a platform cannot be removed once added. **Tests (RC)** now runs four actions from one `rc/*` tag: **Test - macOS** and **Archive - macOS** (both scheme `KeeForgeMac`; archive prepares for App Store Connect, Build For *Any Mac*, native rather than Mac Catalyst) alongside the existing iOS pair, with both test actions Required to Pass. The **TestFlight External Testing - iOS** post-action was deleted, so no platform auto-distributes to external testers any more and every external release is a manual decision. An empty **KeeForge Mac Test** external group exists for the native build. Both cloud secrets render redacted, confirming the Secret flag the earlier audit could not verify; **Restrict Editing** stays off.
  ...
  - Setup candidate `rc/1.15.0-b3` exists for exactly this purpose: repo build 3 on all four targets, no product change, pushed only to prove the four-action workflow and both GitHub Actions gates start and that both archives reach their TestFlight lists. It is not a release candidate. The macOS App Store version record was corrected from the auto-created 1.0 to 1.15.0 so a Mac build can attach to it.
  - The tag started all three gates as designed. Xcode Cloud build 53 ran `Test - iOS`, `Test - macOS`, `Archive - iOS`, and `Archive - macOS` in parallel from one tag, and `macos-rc-tests.yml` fired against an `rc/*` tag for the first time. Check URLs: [Xcode Cloud build 53](https://appstoreconnect.apple.com/teams/d9826238-924a-40e8-90dc-d1ae4bbb19dd/apps/6759309295/ci/builds/64184a97-fa23-474d-bf2a-13d624f99ba9/summary), [iOS 18 RC Tests](https://github.com/KeeForge/KeeForge/actions/runs/34055055898), [macOS RC Tests](https://github.com/KeeForge/KeeForge/actions/runs/34055055905). Still to confirm: the three verdicts, and that both processed TestFlight builds show this tag and SHA — record their separate build numbers then.
