...
- [x] **2. Add Mac archive/TestFlight delivery to Xcode Cloud's `Tests (RC)` workflow.** Configure App Store Connect/Xcode Cloud and document the final action graph in `.agents/skills/release/xcode-cloud-setup.md`.
  ...
  - [x] Immutable `rc/1.16.0-b48` at `a48d6b1` proved the current delivery path. Xcode Cloud build 54 uploaded both iOS and macOS `1.16.0 (54)`, each processed **Complete** and **Validated**; `macos-rc-tests.yml` passed. Xcode Cloud Test - iOS and iOS 18 RC Tests are still running, so they are not accepted and external distribution remains blocked.
  - The processed builds, RC tag/SHA, and cloud URLs are recorded in the candidate manifest. This closes archive/TestFlight delivery, not the package-10 acceptance gates or soak.
