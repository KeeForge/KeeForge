...
- [ ] **10. Cut one RC and produce all three artifacts.** Current candidate: immutable `rc/1.16.0-b50` at `a1ec8f47bf7b48bd5928a2d5b2165b04b19a480d`. Xcode Cloud uploaded iOS and macOS TestFlight `1.16.0 (56)`; both are **Complete** and **Validated**. Both exact Mac export checks passed.
  - The iOS and Mac KDBX gates are accepted. GitHub macOS accepted its canonical verdict: 1,695 passed, 22 skipped, 0 failed. Xcode Cloud is accepted after both canonical SE3 keyboard-focus failures passed locally on iOS 26.5 (23F77). GitHub iOS 18 is accepted after both recycle-bin hit-testing failures passed locally on SE3/iOS 18.6 (22G86); its unit and both iPad lanes passed. Exact source, runtime, selected-test and canonical-result evidence is recorded in the RC50 manifest.
  - On September 14 the owner explicitly authorized TestFlight first, deferring the Mac smoke and final direct signature. Exact iOS and macOS `1.16.0 (56)` are submitted to their public external groups and both show **Waiting for Review**, with automatic tester notification enabled. This candidate-specific exception is recorded in the manifest; the Mac smoke is not marked passed.
  - The local Mac smoke still needs Automation Mode owner authentication. The direct exact ZIP is notarized and stapled; Sparkle `sign_update` remains pending Keychain authorization.
