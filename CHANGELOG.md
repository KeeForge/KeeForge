...
- Xcode Cloud archives/uploads may run automatically, but external TestFlight distribution is manual only after all cloud and local gates and both exact Mac artifact checks are accepted. A named candidate Beta App Review or distribution action may proceed when already authorized in the current task; production go and legal declarations remain separate.
- App Store Connect holds one KeeForge app record with separate iOS and macOS version records, builds, screenshots, metadata, and review submissions. Adding macOS already established universal purchase; hold both with manual release for the first coordinated launch, with phased release still optional and unchosen.
...
- [x] **2. Add Mac archive/TestFlight delivery to Xcode Cloud's `Tests (RC)` workflow.** Configure App Store Connect/Xcode Cloud and document the final action graph in `.agents/skills/release/xcode-cloud-setup.md`.
  ...
  - Historical setup candidates exposed the closed 1.15.0 iOS train and the unsupported Xcode Cloud macOS test-host configuration; the version record is now 1.16.0, `Test - macOS` is intentionally covered by GitHub Actions, and `macos-rc-tests.yml` runs on macos-26.
  - [x] Immutable `rc/1.16.0-b48` at `a48d6b1` proved only archive delivery: Xcode Cloud build 54 uploaded both iOS and macOS `1.16.0 (54)`, each processed **Complete** and **Validated**; `macos-rc-tests.yml` passed. The candidate was rejected and never distributed after reproduced Xcode Cloud creation failures and canonical iOS 18 XCTest UI failures hidden by a green workflow verdict. Its replacement, immutable `rc/1.16.0-b49` at `eacbc3bb8a1dc8116e339dda1be31534b5bc3310`, has Xcode Cloud build 55 archives/uploads for both iOS and macOS `1.16.0 (55)`, each **Complete** and **Validated**.
  - Separate candidate manifests preserve each candidate's processed builds, RC tag/SHA, and cloud URLs; upload success alone does not close package-10 acceptance gates or soak.
...
- [ ] **8. Create the one-time App Store Connect Mac setup.** The account-level setup is done — adding macOS to app `6759309295` established universal purchase; there is no separate app record or bundle identity. The **KeeForge Mac Test** external group exists, and the current processed candidate is `1.16.0 (54)`.
...
- [x] **9. Complete Mac listing and reviewer metadata.** Saved on the macOS `1.16.0` version record. Security copy was corrected in all seven locales; the seven final screenshots are uploaded; the remaining release action is attaching the accepted soaked build and staging review.
  ...
  - Name, subtitle, categories and age rating remain app-level and shared with iOS. Age rating stays **Keep Existing Rating**. Export compliance is resolved **No** in the actual TestFlight metadata for both platforms. `BetaPrivacyURL` is saved as `https://keeforge.com/privacy`. A read-only check of the existing published App Privacy record on 2026-09-14 found policy URL `https://keeforge.com/privacy/`, Product Preview **Data Not Collected**, and Data Types **“Data is not collected from this app.”** No App Privacy edit or new declaration was made.
...
- [ ] **10. Cut one RC and produce all three artifacts.** Immutable `rc/1.16.0-b48` at `a48d6b1` was rejected and never distributed: its Xcode Cloud creation failures were reproduced, and the green iOS 18 GitHub Actions verdict hid canonical XCTest UI failures. Immutable `rc/1.16.0-b49` at `eacbc3bb8a1dc8116e339dda1be31534b5bc3310` was also withheld from distribution. Xcode Cloud build 55 archived/uploaded both iOS and macOS `1.16.0 (55)`, each **Complete** and **Validated**. Its three Xcode Cloud XCTest failures passed exact local reproductions, but two canonical GitHub iOS 18.6 UI failures remain unresolved on that candidate. Build 50 carries focused UI-readiness and sort-precondition fixes and must pass fresh gates. Build 50's local Mac smoke failed `testEscapeInUnlockReturnsToDatabaseList`: opening a database from the sidebar could leave keyboard focus in the sidebar, so typing and Escape never reached the password field. The fix (unlock field takes focus when it appears and keeps it against the sidebar's hand-off, never against a deliberate focus change) needs a new RC.
  ...
  - Fresh iOS and Mac KDBX gates each passed 19 tests with zero failures/skips and 33 KeePassXC artifacts. The local Mac smoke (14), What’s New (1), and screenshot (1) runs passed with zero failures/skips and restored state; GitHub macOS accepted its canonical verdict (1,695 passed, 22 skipped, 0 failed). Both exact exported Mac apps passed their universal channel verifiers.
  - Obtain/export the exact MAS `.app` from the accepted Xcode Cloud archive without rebuilding it. Run `ci_scripts/verify_mac_artifact.sh --channel mas --app <exact-exported-mas-app> --architectures arm64,x86_64 --expect-version {version} --expect-build {macTestFlightBuild}`, then run the locked `build_mac_direct.sh --archive-export` and unlocked `--finalize` phases from the same clean SHA before verifying its exact exported app with `--channel direct --expect-version {version} --expect-build {repoBuild}`. Both must pass before external distribution or direct-artifact staging; a different architecture set requires an explicit product decision recorded in the manifest.
  - The direct b49 artifact is notarization-accepted, stapled, Gatekeeper-accepted, and plain-`unzip` verified. Its saved Sparkle signature independently verifies against the final ZIP and embedded public key. Neither b49 platform build has been added to an external group or submitted for beta review.
...
- [x] **11. Produce and review the final Mac screenshots.** Fresh b48 capture ran one test with one pass and zero skipped captures. All seven 2880×1800 English screenshots were uploaded in listing order, then reloaded; Simplified Chinese inheritance was verified. Fresh b49 capture confirms those saved screenshots remain valid: there are no Mac visual changes. App Store Connect reports no missing Mac display-size assets.
...
- [ ] **13. Soak the exact RC in parallel and make one go/no-go decision.** After all cloud and local gates and both exact-artifact checks are accepted, distribute each exact iOS and Mac build when that named candidate action is already authorized in the current task; otherwise obtain action-time confirmation. Test the staged direct artifact on clean Apple-silicon and Intel hardware/VMs plus the full Sparkle cycle.
...
- [ ] **14. Stage and hold both App Store versions.** After the go decision, use `publish-app-store-version` once per platform to attach the exact soaked iOS and Mac TestFlight build numbers; stage each platform independently through **Ready for Review**, then stop. The immutable RC tag/SHA is the handoff, and `v{version}` does not exist yet.
  ...
  - For the first build of each marketing version/platform, proceed with Beta App Review or external tester distribution when that named candidate action is already authorized in the current task; otherwise obtain action-time confirmation. Keep an approved platform held until the coordinated release decision; never substitute the newest unsoaked build.
