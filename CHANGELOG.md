...
- Keep the legacy iPad-on-Mac availability through native Mac launch and a verified transition; decide whether to withdraw it only afterwards. The default Mac artifact target is universal `arm64,x86_64`; an arm64-only exception requires an explicit product decision and matching support copy.
...
- [ ] **5. Run the production-state migration probe before writing migration code.** On an Apple-silicon Mac, install the current App Store "Designed for iPad" build and seed local, external-file, key-file, stored-key, WebDAV, Dropbox, OneDrive, multi-database, quick-launch, and AutoFill cases.
  - Use a backed-up/snapshotted, sacrificial, or otherwise recoverable sequential harness. Do not attempt an unprotected side-by-side native install: both apps use `com.keevault.app` and both extensions use `com.keevault.app.autofill`, so installation may replace or rebind the legacy app, container, or provider. Preserve the legacy source before installing the native build, then record what the native app can actually read.
  - Specifically record whether iOS-created database/key-file bookmarks resolve under the native sandbox, how both apps register `com.keevault.app.autofill`, and whether cloud account/token records are visible. Do not log database contents, keys, tokens, or passwords.
  - Put the observed compatibility matrix and resulting migration rules in `KeeForgeMac/README.md`. Done when every fixture has a known native outcome: automatic, requires file re-selection/reconnection, recoverable as local cache, or manual export only.
  - **Probe status.** Nothing in the production fixture matrix has been observed. A read-only inventory on 2026-09-02 (Apple silicon, macOS 26.6.2 build 25G83) found the App Store "Designed for iPad" build at `/Applications/KeeForge.app` — `1.15.0` build `52`, `iphoneos`, arm64, `UIDeviceFamily=[1,2]`, team `V82M9YX8BR`, App Group `group.com.keevault.shared`, keychain groups `com.keevault.sharedkeychain` and `com.microsoft.adalcache`, embedded extension `com.keevault.app.autofill`, no `_MASReceipt/receipt` inside the bundle (a filesystem result only; it establishes nothing about account state). It was never launched or seeded. **As of 2026-09-08 that bundle is no longer present on the machine**, so even the downloaded-artifact half of the probe would have to be re-established before seeding starts.
  - The existing `com.keevault.app` container has no files in `Data/Documents`; the shared App Group exists but has no `database-list.json` and no `databases/`, and `cloud-cache/` holds one account directory containing zero files. No database, key, account, token, password, or Keychain value was read. Coexistence of a native development install with the App Store iPad app was deliberately not tested: both apps use `com.keevault.app` and both extensions use `com.keevault.app.autofill`, so installing one can replace or rebind the other's app, container, or provider registration.
  - Fixture-only outcomes that are **not** production migration outcomes, from the native Mac suite: legacy shared-reference migration and idempotency (`DatabaseReferenceMigrationTests`), scoped and plain bookmark resolution (`SecurityScopedBookmarkManagerTests`), App Group write guardrail (`AppGroupGuardrailTests`), and AutoFill status state model (`AutoFillStatusServiceTests`) all pass. Real stored-composite-key and cloud-token visibility stay **unproven** — the suite recorded Keychain write skips (`-25308`) and no production secret was accessed.
  - Every row below is **not observed** and must not be promoted to automatic, file re-selection, reconnection, cache recovery, or manual export until a real transition is exercised in a backed-up, snapshotted, or otherwise recoverable sequential harness:

| Existing iPad-on-Mac fixture | Native outcome | What remains required |
| --- | --- | --- |
| Local database | Not observed | Install the App Store build, create/open a local fixture, preserve the source, then test the native app; do not assume side-by-side coexistence |
...
| AutoFill enabled/disabled and registration | Not observed | Seed both states, inspect provider registration in System Settings/PlugInKit, then re-enable the native provider and check for a competing legacy registration |
  ...
  - Until that matrix is populated, the migration rules stay the pre-implementation ones: local and external files need export or re-selection when a bookmark does not transfer, WebDAV needs reconnecting, Dropbox/OneDrive can only be offered as local recovery when a valid encrypted cache is present, stored-key and AutoFill preservation need explicit native readback, and no source container is ever deleted or overwritten.

- [ ] **6. Implement an idempotent native first-launch migration from that matrix.** Add a macOS migration service beside `KeeForge/Services/Persistence/DatabaseListStore.swift`/`SharedVaultStore.swift`, invoke it before the database list is presented, and store only its migration-version/result state in native `UserDefaults.standard`.
  - Never delete or overwrite the iPad app's source/container. Do not duplicate a reference the native app can already use. Commit a migrated local copy only after its encrypted KDBX bytes are durably written and re-readable.
  - Prompt once to reselect external databases and key files whose bookmarks cannot transfer; preserve UUID/nickname/read-only/quick-launch/AutoFill/stored-key metadata where safe. WebDAV may require reconnecting the account. Convert recoverable Dropbox/OneDrive caches into new local-only references with cloud metadata removed and a persistent "not syncing" explanation.
  - Present a per-database result (`ready`, `needs file`, `needs key file`, `reconnect WebDAV`, `local recovery copy`, or `manual export`) with retry. Re-enabling AutoFill must point users to System Settings → General → AutoFill & Passwords and detect/report a competing legacy provider registration.
  - Preserve the App Group guardrail: only encrypted KDBX bytes, bookmark blobs, and filename metadata may enter shared storage; keys remain in Keychain. Update the closest `AGENTS.md`/README maps for any new files.

- [ ] **7. Gate migration and document recovery.** Add focused shared tests under `KeeForgeTests/` so they also run as `KeeForgeMacTests`; add only the smallest Mac UI smoke needed for first-launch/result/retry behavior.
  - Cover: rerun/idempotency, interrupted copy, corrupt/missing cache, duplicate reference, failed bookmark selection, key-file selection, stored composite key, WebDAV reconnect, Dropbox/OneDrive local conversion with no sync metadata, multiple databases, quick launch, and AutoFill disabled/enabled states.
  - Run the relevant migration tests, `AppGroupGuardrailTests`, `SecurityScopedBookmarkManagerTests`, both KDBX compatibility gates if cache-copy semantics change, and a recoverable sequential transition. Do not mark a production outcome automatic, reselect/reconnect, cache recovery, or manual export until the real probe observes it.
  - Publish keeforge.com instructions covering backup/export first, each result state, WebDAV reconnection, the Dropbox/OneDrive limitation, AutoFill re-enable, retry/manual export, and the exact preservation limits established by the production probe. Do not promise that the old container survives a native install; do not withdraw the iPad app's Mac availability before the native app is live and verified.
