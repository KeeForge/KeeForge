...
- `SearchResultsMoveUITests` — moving an entry out of the search results (#124): the long-press menu offers `entry-row.move-context`, the destination picker presents, and the entry both lands in the chosen group and leaves its old one, plus the cancel path leaving it where it was. Move to Group reached the shared `EntryListView` only with the macOS parity pass, having been wired into `GroupListView`'s own row builder when it was added, so v1.15.0 shipped with the item missing from search results entirely. Every shell still builds that item itself, so this is what stops it going missing again. Shares `searchForEntries` / `dismissSearch` with `SearchResultsDeleteUITests` through `EntryEditUITestCase`
