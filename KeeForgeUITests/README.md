...
- `TOTPEnrollmentUITests` — entry-editor TOTP enrollment happy path using the `autofill-union` fixture's "Union Bank" entry (no TOTP): edit → `entry-edit.totp.enter-key` → reveal via `entry-edit.totp.secret-visibility-button` (the simulator has no passcode, so the device-owner gate falls through) → type a Base32 secret into `entry-edit.totp.secret-field` → save → `entry.totp.code` renders 6 numeric digits. QR scanning is not covered — XCUITest cannot drive the simulator camera
