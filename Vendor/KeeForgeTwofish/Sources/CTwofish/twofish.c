 ...
 */
/* KEEFORGE INTEGRATION: report fatal self-test failures without hanging. */
extern _Noreturn void kf_twofish_report_fatal( const char * message );
#define Twofish_fatal( msg )      {kf_twofish_report_fatal( msg );}
...
void Twofish_prepare_key( Byte key[], int key_len, Twofish_key * xkey )
    ...
    /* KEEFORGE INTEGRATION: use non-optimizable clearing for key scratch. */
    extern void kf_twofish_secure_zero( void * buffer, size_t length );
    kf_twofish_secure_zero( K, sizeof( K ) );
    }
