...
suggest the next minor bump (e.g. 1.5.0 → 1.6.0). Ask the user to confirm before proceeding.
...
Both targets must always have identical version values. Use the Edit tool to make these
changes — there are exactly 4 values to update (2 × MARKETING_VERSION, 2 × CURRENT_PROJECT_VERSION).
...
Then run **all** unit tests and UI tests:
...
`v{version}` tag and start the build → test → TestFlight → App Store Review pipeline.
