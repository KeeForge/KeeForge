...
- `-only-testing:` takes test **class** names; the map below lists files, and the two sometimes differ: `PasskeyTests.swift` declares `PasskeyCredentialTests` and `PasskeyCryptoTests` (no `PasskeyTests` class), and `DatabaseViewModelTests.swift` also declares `CloudSyncCoordinatorTests` (around line 2222).
