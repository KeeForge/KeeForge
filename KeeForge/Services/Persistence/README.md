...
- `LocalDatabaseSaver.swift` handles atomic local saves, open-time conflict detection, backup rotation (timestamped backups, pruned via `DatabaseListStore.pruneBackups` to keep the 5 newest), and shared-cache refresh.
- `SharedVaultStore.swift` owns the App Group cache directory paths and reads the shared cached database copy for the app and AutoFill extension. Production cache *writes* go through `DatabaseListStore.cacheDatabaseCopy(_:for:)` (called from `LocalDatabaseSaver`, `CloudSyncCoordinator`, `DatabaseCreationService`, and `DatabaseViewModel`); `SharedVaultStore.cacheDatabaseCopy(_:sourceURL:)` is a legacy API whose only remaining callers are tests.
