...
- Verification-code enrollment flow: an incoming `otpauth://` link runs `App/KeeForgeApp.swift` → `Models/OTPAuthURI.swift` → `ViewModels/TOTPEnrollmentViewModel.swift` → `Views/TOTPEnrollmentDestinationView.swift` → `Views/EntryEditView.swift` → `Models/DatabaseDraft.swift`.
