...
- `DatabaseListView.swift`, `DatabaseRowView.swift`, the in-file `DatabaseDetailsView`, and `CloudFileBrowserView.swift` own database picking, cloud browsing, rename/remove actions, read-only toggles and badges, pending-upload badges/conflict copy, "Push pending changes" actions, settings entry points, and the Dropbox write-scope reconnect banner shown after the cloud-save upgrade.
