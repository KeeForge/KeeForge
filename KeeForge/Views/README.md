...
- `GroupListView.swift`, `EntryListView.swift`, `EntryDetailView.swift`, and `SearchView.swift` own post-unlock navigation, entry/group creation and deletion affordances, expired-entry list/detail warnings, enabled expiration timestamps, detail presentation, and the global read-only + unsaved-change surfaces shown while a database is open. macOS caveats: pushed `NavigationStack` levels inside the `NavigationSplitView` sidebar render zero-height on macOS, so the Mac sidebar uses flat drill-down (`GroupListView.onSelectGroup`/`onNavigateBack` + a `group.back` toolbar button) driven by `RegularDatabaseWorkspaceView`; the entry editor is presented as a sheet on macOS instead of a navigation push; `.searchable` attaches only at the vault root there (⌘F focuses it via `searchFocused`, macOS 15+).
