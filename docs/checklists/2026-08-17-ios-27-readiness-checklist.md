...
- Already clean, no action: zero `PreviewProvider` (all `#Preview`), zero `ObservableObject`/`@Published`, zero `NavigationView`, all 49 `onChange(of:)` sites use the two-parameter signature, zero `.roundedBorder`/`.squareBorder` text-field styles, zero MetricKit, zero On-Demand Resources, zero deprecated status-bar accessors.

## 4. AutoFill / password-manager surface

- [ ] **Re-test the credential identity store on iOS 27 GM.** The iOS 27.0-beta enumeration bug (objects not conforming to `ASCredentialIdentity`, trapping the NSArray bridge) is already worked around by `droppingNonConformingIdentities` (`CredentialIdentityStoreManager.swift:119-129`). Keep the workaround — it's harmless when fixed — but confirm the GM behavior and file a Feedback Assistant report if it's still broken.
...
- [x] **No `PrivacyInfo.xcprivacy` existed in any target.** Fixed 2026-08-17: `KeeForge/Resources/PrivacyInfo.xcprivacy` (both app targets) and `AutoFillExtension/PrivacyInfo.xcprivacy` (both extensions), placed so the existing folder globs bundle them without touching the order-locked AutoFill allow-lists. Declarations from a full required-reason audit: UserDefaults (`CA92.1` + `1C8F.1` — standard defaults plus the App Group suite, which reaches the extensions via `SharedVaultStore`/`SettingsService`), file timestamps (`DDA9.1`/`C617.1`/`3B52.1` app; `C617.1` extension via `FaviconService` cache TTL); no disk-space, boot-time, or active-keyboard API use anywhere; `NSPrivacyTracking = false`. Collected data is the feedback form only (email, message, diagnostics, attached photo) — the photo uses Apple's specific `NSPrivacyCollectedDataTypePhotosorVideos` type rather than the generic user-content bucket. Mirror all four in the App Store Connect nutrition label; re-check `NSPrivacyCollectedDataTypeLinked = false` on the email against how `feedback.keeforge.com` actually retains it, since the email rides in the same JSON object as the message and photo. Verified 2026-08-16 from a device build: SwiftyDropbox 10.2.4 and MSAL 1.9.0 each ship their own manifest (`SwiftyDropbox_SwiftyDropbox.bundle/`, `MSAL.framework/`), so their declarations — including the linked email/account data their SDKs collect — aggregate on top of ours rather than needing to be restated.
- [x] **`UIRequiredDeviceCapabilities = ["armv7"]`** — fixed 2026-08-17, now `arm64` (safe: every iOS 18 device is arm64).
