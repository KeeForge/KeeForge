...
- [ ] **`NSAppTransportSecurity` sets `NSAllowsArbitraryLoads = true`** alongside per-domain exceptions (`KeeForge/Info.plist:105-159`). This contradicts the repo's limited-egress posture; the domain exceptions should suffice — try removing the blanket allowance and test WebDAV against plain-HTTP servers (the likely reason it exists; if HTTP WebDAV must stay supported, scope it with `NSAllowsArbitraryLoads` documented, or gate on `NSAllowsLocalNetworking`).
