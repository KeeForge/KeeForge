...
- **Dropbox write scope.** Slice 05 adds `files.content.write` to the OAuth scope request. **Existing Dropbox-connected users will need to re-authorize** to grant write access. The `DropboxCloudProvider` must detect "missing scope" errors on first write and present a re-auth flow that explains why.
...
- **Slice 05's launch behavior:**
  ...
  - **Proactive re-auth banner.** On main app launch, if any `DatabaseReference` is `.cloud(.dropbox)` and the stored token does not have write scope, present a one-time post-update banner explaining what changed and offering "Reconnect Dropbox". Users who dismiss the banner still get the same offer the moment they try to save. Users who have no need to edit can ignore the banner indefinitely.
  - **Save fails gracefully without write scope.** If the user tries to save before re-authorizing, slice 05 surfaces `CloudProviderError.writeScopeRequired` and slice 06's UI presents the same reconnect prompt with the entry's draft preserved.
