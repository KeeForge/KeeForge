...
- If Dropbox reports a missing write scope during upload, surface `CloudProviderError.writeScopeRequired` at the save attempt. Older proactive upgrade-reminder state was retired after the write-scope release had been in the field for several versions.
- **Reads keep working without re-auth.** This slice must not break any read path. Existing refresh tokens authenticate metadata fetches and downloads exactly as before. If an old token lacks write access, reads retain access to all Dropbox-backed databases. The only thing that breaks is the save action, which surfaces `writeScopeRequired` at the moment of failure.
...
- **Modified:** `KeeForge/Services/Cloud/DropboxCloudProvider.swift` — implement `upload(...)`, bump scope request, and map missing write-scope upload failures.
...
- [ ] **Reads keep working without re-auth.** Manual verification: install the new build over a build that only has read scope; open a Dropbox-backed database; assert the open succeeds and AutoFill from QuickType works. Only the save path should surface missing write scope.
- [ ] No force unwraps.
